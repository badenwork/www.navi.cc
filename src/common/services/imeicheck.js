...
;(function(){
    ...
    // Можно использовать валидацию в полях формы
    .directive('imeicheck', function() {
        return {
            require: 'ngModel',
            link: function(scope, elm, attrs, ctrl) {
                ctrl.$parsers.unshift(function(imei) {
                    if(imeicheck(imei)) {
                        ctrl.$setValidity('imei', true);
                        return imei;
                    } else {
                        ctrl.$setValidity('imei', false);
                        return imei;                        // На данный момент мы разрешаем использовать неправильные IMEI
                    }
                });
            }
        };

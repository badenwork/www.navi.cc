/* global angular:true, google:true, $:true, moment:true, xlsx:true */

// getXLSXDownloadLink в глобальной области видимости.
(function(){
...
.controller('ReportsViewCtrl', ['$scope', '$location', 'account', '$http', 'SERVER', 'GeoGPS', 'System', 'systems', '$route',
    function($scope, $location, account, $http, SERVER, GeoGPS, System, systems, $route) {
        ...
        $scope.getFuel = function(data) {
            ...
            window.console.log('liters ', liters);
            return liters;
        ...
        $scope.formatPosition = function(index) {
            ...
            $scope.geocoder.geocode({
                    'latLng': new google.maps.LatLng($scope.report.reportData.rows[index].lat, $scope.report.reportData.rows[index].lon)
                },
        ...
        $scope.parseData = function(data) {
            ...
            //gererate filename
            $scope.report.title = $scope.systems[$scope.report.systemKey].title + '_' + moment($scope.report.interval.start).format('DD/MM/YYYY') + '_' + moment($scope.report.interval.end).format('DD/MM/YYYY');
            ...
            for (i = 0; i < $scope.report.reportData.rows.length; ++i) {
                item = $scope.report.reportData.rows[i];
        ...
        $scope.report.generateReport = function() {
            ...
            var day = {
                date: $scope.report.interval.end
            };
            ...
            while (cmpStr != day.date.toDateString()) {
                ...
                day = {
                    date: moment(day.date).subtract('d', 1).toDate()
                };
            }
...
function getXLSXDownloadLink(report) {
    ...
    for (var i = 0; i < days.length; ++i) {
        ...
        reportDataPerDay.push([{
            colSpan: 8,
            value: days[i].strFormatted
        }]);

        rawData = days[i].events;
    ...
    // отчет общий
    rawData = report.reportData.rows;
